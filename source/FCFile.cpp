...
namespace FaceCat{
	...
	bool FCFile::getDirectories(const char *dir, vector<string> *dirs){
		long hFile = 0;  
	...
	bool FCFile::getDirectories(const String &file, vector<String> *dirs){
		...
		for (int i = 0; i < size; i++){
			dir = FCStr::stringTowstring(vectors[i]);
			dirs->push_back(dir);
		}
		return res;
	}
	...
	bool FCFile::getFiles(const char *dir, vector<string> *files){
		long hFile = 0;  
		struct _finddata_t fileinfo;  
		string p;  
		if((hFile = (long)_findfirst(p.assign(dir).append("\\*").c_str(),&fileinfo)) !=  -1){  
	...
	bool FCFile::getFiles(const String &dir, vector<String> *files){
		String file;
		vector<string> vectors;
		string sDir = FCStr::wstringTostring(dir);
		...
		int size=(int)vectors.size();
		for (int i = 0; i < size; i++){
			file = FCStr::stringTowstring(vectors[i]);
